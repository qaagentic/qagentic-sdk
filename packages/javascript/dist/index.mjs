...
function attachFile(filePath, name) {
  ...
  const displayName = name || path2.basename(filePath);
  if (!fs3.existsSync(resolvedPath)) {
    return attach(filePath, displayName, "text/plain");
  }
  const content = fs3.readFileSync(resolvedPath);
...
function attachScreenshot(data, name = "Screenshot") {
  if (typeof data === "string" && fs3.existsSync(data)) {
    const content = fs3.readFileSync(data);
...
var JSONReporter = class {
  ...
  startRun(run) {
    ...
    if (this.config.local.cleanOnStart) {
      const files = fs3.readdirSync(this.outputDir);
      for (const file of files) {
        if (file.endsWith(".json")) {
          fs3.unlinkSync(path2.join(this.outputDir, file));
        }
      }
    }
  }
  endRun(run) {
    ...
    fs3.writeFileSync(runFile, JSON.stringify(this.serializeRun(run), null, 2));
    const testsDir = path2.join(this.outputDir, "tests");
    if (!fs3.existsSync(testsDir)) {
      fs3.mkdirSync(testsDir, { recursive: true });
    }
    for (const test of run.tests) {
      const testFile = path2.join(testsDir, `${test.id}.json`);
      fs3.writeFileSync(testFile, JSON.stringify(this.serializeTest(test), null, 2));
    }
  }
  reportTest(_test) {
  }
...
var JUnitReporter = class {
  ...
  endRun(run) {
    ...
    fs3.writeFileSync(junitFile, xml);
  }
  reportTest(_test) {
  }
  generateXml(run) {
...
export { QAgenticReporter, Severity, Status, Step, VERSION, attach, attachFile, attachHtml, attachJson, attachScreenshot, attachText, attachVideo, configure, description, epic, feature, getConfig, getReporter, issue, label, layer, link, owner, parentSuite, severity, step, story, subSuite, suite, tag, testcase, title };
